...
def getCVADistance(Backgrounds, curFrame):
    cva_dist = cva_compute(Backgrounds, curFrame)
...
def getDist( Backgrounds, IM, Gmag, cva_distance, R, H, W, N):
    sumBW = np.zeros( [H, W], dtype=np.float32)
    minDist=1000*np.ones( [H, W], dtype=np.float32);

    for j in range(N):
        dist = ( np.abs( np.subtract(Backgrounds[:,:,j],IM) ) + Gmag + cva_distance);
        minDist[np.where(dist<minDist)] = dist[np.where(dist<minDist)];

        BW = 1.0*np.logical_and( (dist>=R) , (np.abs( np.subtract(Backgrounds[:,:,j],IM)) >1 )  );
        #print(BW.shape)
        sumBW=sumBW+BW;
