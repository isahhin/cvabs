...
def processFolder(datasetPath, binaryRootPath):
    ...
    for category in getDirectories(datasetPath):
        ...
        for video in getDirectories(categoryPath):
            ...
            H = np.int(1280)
            W = np.int(720)       
            pil_image = Image.fromarray(imgTest)
            pil_image= pil_image.resize( (W, H), Image.BICUBIC)
            imgTest = np.array(pil_image)
            ...
            for i in range(0,N):
                ...
                pil_image = Image.fromarray(curFrame)
                pil_image= pil_image.resize( (W, H), Image.BICUBIC)
                curFrame = np.array(pil_image)
            ...
            for file in allFiles:
                ...
                cva_distance = getCVADistance( Backgrounds, curFrame )
                
                Gmag = getGradientDistance( curFrame, MB)
                Gmag = np.absolute(Gmag) 
  
                #get segmented image
                [BW, minDist] = getDist( Backgrounds, curFrame, Gmag, cva_distance, R, H, W, N)  
...
if __name__ == "__main__":
    ...
    binaryRootPath = "D:/DATASETS/dataset2014/results/SWCD_2019_08_01";
